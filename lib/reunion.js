...
const HTML_BUILDER = {
    ...
    listItem(ordinal, html) {
        if (!ordinal)
            ordinal = '▪';
        return `<div class='list-item'><span class='ordinal'>${ordinal}</span><span class='content'>${html}</span></div>`;
    },
    ...
    linkIcon(url, className, target) {
        if (Array.isArray(className))
            className = className.join(' ');
        className += ' icon';
        return HTML_BUILDER.link('📘', url, className, target); //➤
    },
    moreLink() {
        return HTML_BUILDER.link('(meer)', ...arguments);
    },
